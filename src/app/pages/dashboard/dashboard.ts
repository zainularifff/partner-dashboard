...
export class DashboardComponent implements OnInit {
  ...
  // ✅ FIXED: Gunakan satu variable 'loading' sahaja untuk HTML dan TS
  loading = false;
  ...
  // ✅ Isytihar variable ini supaya boleh diakses oleh generateServerHealthSpinners
  vendorRows: any[] = [];
  ...
  // ================= VENDOR ROWS (ASAL) =================
  osRows: any[] = [
    { osName: 'Windows Server 2022', detectionTime: '2.4h', riskScore: 92, color: 'green' },
    { osName: 'Ubuntu 22.04 LTS', detectionTime: '4.8h', riskScore: 78, color: 'orange' },
    { osName: 'CentOS 7', detectionTime: '8.2h', riskScore: 65, color: 'red' },
  ];

  // ================= LEASES =================
  leases = [
    {
      name: 'KPM JKR HRPZ II',
  ...
  // ================= GETTER: HIGHEST RISK BRAND =================
  // ✅ Mencari brand dengan peratusan critical paling tinggi secara automatik
  get highestRiskBrand() {
    if (!this.brandRows || this.brandRows.length === 0) {
      return { name: 'N/A', critical: 0 };
    }
    // Salin array dan susun mengikut nilai critical (menurun)
    const sorted = [...this.brandRows].sort((a, b) => (b.critical || 0) - (a.critical || 0));
    return sorted[0];
  }
  ...
  loadDashboard(clientId: string = ''): void {
    // 🚀 Mula loading sebelum tarik data server
    this.loading = true;
      ...
      .subscribe((clients) => {
        forkJoin({
          ...
          next: ({ summary, faults, aging, assetSummary, assetsPerClient }: any) => {
            ...
            // ✅ Selesai loading data (termasuk dari .70/TCO3)
            this.loading = false;
  ...
  generateServerHealthSpinners(summary: any): void {
    /* // COMMENTED OUT: Logik dinamik ditutup sementara. 
    ...
    */
    ...
    // ✅ DATA STATIC (Guna ini supaya UI tidak berubah bila filter ditukar)
  ...
  // Tambah di dalam class DashboardComponent
  getLegendLabel(item: string): string {
