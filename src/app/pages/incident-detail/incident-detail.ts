...
export class IncidentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // 🔥 AMBIL TYPE DARI HISTORY.STATE
    const navState = history.state;

    if (navState && navState.type) {
      this.type = navState.type;
      this.selectedServers = navState.activeFilter
        ? navState.activeFilter.split(',')
        : [];
    } else {
      this.type = 'open'; // fallback kalau refresh
    }
    ...
    this.api.getFullList().subscribe({
      next: (rows: any[]) => {
        ...
        this.allData = rows.map((x) => {
          ...
          let endTime: number;
          if (statusNum === 2 && x.completed_time && Number(x.completed_time) > 0) {
            endTime = Number(x.completed_time);
          } else {
  ...
  calculateDynamicStats() {
    ...
    const baseDataByType = this.allData.filter((x) => {
      ...
      if (this.type === 'pending') return x.statusCode === 1;
    ...
    this.serverStats = Array.from(serverMap.entries()).map(([name, count]) => {
      ...
      let colorClass = name.toLowerCase().includes('client') ? 'orange' : 'blue';
  ...
  applyFilter() {
    ...
    this.filteredData = this.allData.filter((x) => {
      ...
      else if (this.type === 'pending') matchType = x.statusCode === 1;
